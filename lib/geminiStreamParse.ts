/**
 * Parse Gemini `streamGenerateContent` responses (SSE via ?alt=sse).
 * Each event often carries cumulative text; we emit only new suffix deltas.
 */
...
/** Fallback when `alt=sse` is not used: one JSON object per line. */
