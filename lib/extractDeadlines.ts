...
/**
 * Read all text from a PDF file using pdfjs-dist.
 */
...
/**
 * Try the Gemini-powered extractor first, then fall back to the local
 * heuristic if the API isn't configured or the call fails.
 */
...
/**
 * Local fallback: lines that mention deadline-keywords AND parse to a date.
 *
 * Uses the academic term inferred from the document text (looking for tokens
 * like "Winter 2026", "F25", or a "Fall/Winter/Spring/Summer YYYY" phrase) to
 * anchor month-day-only references to the right year, instead of chrono's
 * default behavior of bumping past dates a year forward.
 */
export function extractDeadlinesFromText(
  ...
  for (const snippet of candidates) {
    ...
    // If chrono chose a year that lands outside our inferred term and the
    // snippet didn't include an explicit year, snap into the term window.
...
function inferTerm(text: string, refDate: Date): Term | null {
  // Long form: "Winter 2026", "Fall 2025", "Summer 2026", etc.
  ...
  // Short form: "F25", "W26", "S26" near the start of the document or filename.
  ...
  // Final guess: pick the season around the reference date so chrono parsing
  // is still grounded for syllabi that don't say.
  const m = refDate.getMonth();
  const year = refDate.getFullYear();
  const season =
    m <= 3 ? "winter" : m <= 7 ? "summer" : m <= 11 ? "fall" : "winter";
  return termRange(season, year);
}
...
function snapIntoTerm(d: Date, term: Term): Date {
  if (d.getTime() >= term.start.getTime() && d.getTime() <= term.end.getTime()) {
    return d;
  }
  // Try shifting by ±1 year to land inside the window.
  for (const dy of [-1, 1, -2, 2]) {
    const shifted = new Date(d);
    shifted.setFullYear(shifted.getFullYear() + dy);
    if (
      shifted.getTime() >= term.start.getTime() &&
      shifted.getTime() <= term.end.getTime()
    ) {
      return shifted;
    }
  }
